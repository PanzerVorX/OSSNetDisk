<?php
	...
	function getShowFileSize($fileByte){
		...
		$bucket=@$_GET['bucket']?$_GET['bucket']:$_COOKIE['bucket'];
		$parentPath=@$_GET['parentPath']?$_GET['parentPath']:'';
		...
		$adminTableName=$bucket."_admin";//表名
		$result = $pdo->query("show tables like '". $adminTableName."'");
		$isExistAdminTable = $result->fetchAll();
		if(count($isExistAdminTable)){//若存在则清空表
			$sql="truncate {$adminTableName}";
			$pdo->exec($sql);
		} 
		try {
		    ...
		    if (!empty(count($objectList))) {	
			    ...
			    foreach ($objectList as $objectInfo) {
					...
					//保持用户与管理者查询记录同步
					if(count($isExistAdminTable)){
						$sql="insert into {$adminTableName}  (file_path,file_size,last_date,file_type,file_bucket) values (?,?,?,?,?);";
						$smt=$pdo->prepare($sql);
						$smt->bindParam(1,$object);
						$smt->bindParam(2,$fileSize);
						$smt->bindParam(3,$lastDate);
